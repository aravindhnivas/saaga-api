...
def safe_post_request(endpoint, data, files=None):
    ...
    if res.ok:
        ...
        if isinstance(data, dict):
            logger.info(f"returned ID: {data['id']=}\n\n")
            return int(data["id"])
        else:
            return None
...
def post_payload(payload_list):
    ...
    for payload in payload_list:
        ...
        for meta in payload["species_metadata"]:
            ...
            if meta_id is None:
                logger.error(
                    f"Failed to post metadata for {payload['species']['iupac_name']}"
                )
                continue
            ...
            for ref, meta_ref in zip(meta["reference"], meta["meta_reference"]):
                ...
                if ref_id is None:
                    logger.error(
                        f"Failed to post meta reference for {payload['species']['iupac_name']}"
                    )
                    continue
